...
function computePedidoTotals({
  ...
  const correoValue = Number(correo) || 0;
  const totalConDescuento = descuento > 0 ? subtotal * (1 - descuento / 100) : 0;
  const totalConRecargo = recargo > 0 ? (subtotal + correoValue) * (1 + recargo / 100) : 0;
  const totalConCorreo =
    descuento > 0 ? totalConDescuento + correoValue : recargo > 0 ? totalConRecargo : subtotal + correoValue;
  return {
    subtotal,
    totalConDescuento,
    totalConRecargo,
