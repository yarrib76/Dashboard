...
async function processAbmBatch(conn, ordenCompra, items) {
  ...
  for (const item of items) {
    const observaciones = (item.observaciones || '').toString().slice(0, maxObservaciones);
    ...
    if (Object.prototype.hasOwnProperty.call(item, 'proveedorSku')) {
      updateFields.push('ProveedorSKU = ?');
      updateValues.push(item.proveedorSku || '');
    }
    ...
    updated.push({
      ...
      proveedorSku: item.proveedorSku || '',
    });
  }
