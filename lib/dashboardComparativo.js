...
function buildDashboardComparativoAllYearsPayload(params, rows = []) {
  const { yearA, yearB, monthFrom, monthTo, mode, entity } = params;
  const labels = [];
  for (let month = monthFrom; month <= monthTo; month += 1) {
    labels.push(MONTH_LABELS_SHORT[month - 1] || String(month));
  }
  const years = [...new Set(rows.map((row) => Number(row?.anio)).filter((year) => Number.isFinite(year)))].sort(
    (a, b) => a - b
  );
  const series = years.map((year) => {
    const yearRows = rows.filter((row) => Number(row.anio) === year);
    const built = buildComparativoSeries(yearRows, monthFrom, monthTo);
    return { year, values: built.values };
  });
  const ticketSeries =
    mode === 'cantidad' || mode === 'monto'
      ? years.map((year) => {
          const yearRows = rows.filter((row) => Number(row.anio) === year);
          const monthMap = new Map();
          yearRows.forEach((row) => {
            const month = Number(row?.mes);
            const cantidad =
              mode === 'cantidad' ? Number(row?.valor) || 0 : Number(row?.cantidad) || 0;
            const monto = mode === 'cantidad' ? Number(row?.monto) || 0 : Number(row?.valor) || 0;
            if (!Number.isFinite(month) || !(month >= monthFrom && month <= monthTo) || !(cantidad > 0)) return;
            monthMap.set(month, Number((monto / cantidad).toFixed(2)));
          });
          const values = [];
          for (let month = monthFrom; month <= monthTo; month += 1) {
            values.push(monthMap.get(month) || 0);
